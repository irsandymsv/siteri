...
class honorSemproController extends Controller
{
    ...
    public function ktu_verif(Request $request, $id)
    {
        ...
        if ($request->verif_ktu == 2) {
            ...
            $sk_honor->pesan_revisi = $request->pesan_revisi;
            $sk_honor->save();
            return redirect()->route('ktu.sk-honor-sempro.index')->with("verif_ktu", 'SK berhasil ditarik, status kembali menjadi "Draft"');
        } else if ($request->verif_ktu == 1) {
            $sk_honor->id_status_sk_honor = 4;
            $sk_honor->pesan_revisi = null;
            $sk_honor->save();
            return redirect()->route('ktu.sk-honor-sempro.index')->with('verif_ktu', 'verifikasi SK berhasil, status SK saat ini "Disetujui KTU"');
        }
    }
    ...
    public function wadek2_verif(Request $request, $id)
    {
        ...
        if ($request->verif_wadek2 == 2) {
            ...
            $sk_honor->pesan_revisi = $request->pesan_revisi;
            $sk_honor->save();
            return redirect()->route('dekan.sk-honor-sempro.index')->with("verif_wadek2", 'SK berhasil ditarik, status kembali menjadi "Draft"');
        } else if ($request->verif_wadek2 == 1) {
            $sk_honor->id_status_sk_honor = 5;
            $sk_honor->pesan_revisi = null;
            $sk_honor->save();
            return redirect()->route('dekan.sk-honor-sempro.index')->with('verif_wadek2', 'verifikasi SK berhasil, status SK saat ini "Disetujui Wakil Dekan 2"');
        }
    }
    ...
    public function dekan_verif(Request $request, $id)
    {
        ...
        if ($request->verif_dekan == 2) {
            ...
            $sk_honor->pesan_revisi = $request->pesan_revisi;
            $sk_honor->save();
            return redirect()->route('dekan.sk-honor-sempro.index')->with("verif_dekan", 'SK berhasil ditarik, status kembali menjadi "Draft"');
        } else if ($request->verif_dekan == 1) {
            $sk_honor->id_status_sk_honor = 6;
            $sk_honor->pesan_revisi = null;
            $sk_honor->save();
            return redirect()->route('dekan.sk-honor-sempro.index')->with('verif_dekan', 'verifikasi SK berhasil, status SK saat ini "Disetujui Dekan"');
        }
    }
}
