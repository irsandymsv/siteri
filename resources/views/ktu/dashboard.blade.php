				      	...
				      	@else
	   			      	@foreach ($sk_sempro_dikirim as $item)
	   			      		<tr>
	   			      			...
	   			      			<td>{{ Carbon\Carbon::parse($item->created_at)->locale('id_ID')->isoFormat('D MMMM Y') }}</td>
	   			      			<td>
				      					<a href="{{ route('ktu.sk-sempro.show', $item->no_surat) }}" title="Lihat Detail" class="btn btn-primary"><i class="fa fa-eye"></i></a>
				      				</td>
	   			      		</tr>
	   			      	@endforeach
				      	@endif
				      	...
				      	@else
	   			      	@foreach ($sk_skripsi_dikirim as $item)
	   			      		<tr>
	   			      			...
	   			      			<td>{{ Carbon\Carbon::parse($item->created_at)->locale('id_ID')->isoFormat('D MMMM Y') }}</td>
	   			      			<td>
				      					<a href="{{ route('ktu.sk-skripsi.show', $item->id) }}" title="Lihat Detail" class="btn btn-primary"><i class="fa fa-eye"></i></a>
				      				</td>
	   			      		</tr>
	   			      	@endforeach
				      	@endif
		...
		<div class="col col-md-6">
			<div class="box box-primary">
			  	<div class="box-header with-border">
			   	...
			   	<div class="box-tools">
			   		<a href="{{ route('ktu.honor-sempro.index') }}" class="btn btn-default" title="Lihat Semua SK Sempro">Lihat Semua</a>
			   	</div>
			  	</div>
			  <!-- /.box-header -->
			  <div class="box-body">
				      	...
				      	@else
	   			      	@foreach ($sk_honor_sempro as $item)
	   			      		<tr>
	   			      			...
	   			      			<td>
				      					<a href="{{ route('ktu.honor-sempro.show', $item->id) }}" title="Lihat Detail" class="btn btn-primary"><i class="fa fa-eye"></i></a>
				      				</td>
	   			      		</tr>
	   			      	@endforeach
				      	@endif
			...
			<div class="col col-md-6">
				<div class="box box-danger">
				  	<div class="box-header with-border">
				   	...
				   	<div class="box-tools">
				   		<a href="{{ route('ktu.honor-skripsi.index') }}" class="btn btn-default" title="Lihat Semua SK Skripsi">Lihat Semua</a>
				   	</div>
				  	</div>
				  <!-- /.box-header -->
				  <div class="box-body">
					      	...
					      	@else
		   			      	@foreach ($sk_honor_skripsi as $item)
		   			      		<tr>
		   			      			...
		   			      			<td>
					      					<a href="{{ route('ktu.honor-skripsi.show', $item->id) }}" title="Lihat Detail" class="btn btn-primary"><i class="fa fa-eye"></i></a>
					      				</td>
		   			      		</tr>
		   			      	@endforeach
					      	@endif
